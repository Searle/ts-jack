export default class Default {
    ...
    static complement2_16(value: number): number {
        if (value >= 0) return value;
        else return 0b1111111111111111 - (-value - 1);
    }

    static stringProgramToBin(programString: string) {
        const program = [];

        // CLEANUP
        const splitProgramString = programString.split("");
        let cleanedProgramString = "";
        for (let i = 0; i < splitProgramString.length; i++) {
        ...
        for (let i = 0; i < cleanedProgramString.length / 16; i++) {
            const currentByte = cleanedProgramString.substring(
                16 * i,
                16 * (i + 1)
            );
            const currentNumber = parseInt(currentByte, 2); //;console.log(currentNumber)
    ...
    static KBDscancode(keyCode: string) {
        switch (keyCode) {
            case "0":
                return 48;
            case "1":
    ...
    static hexToRGBA(hex: string) {
        if (/^#([A-Fa-f0-9]{3}){1,2}$/.test(hex)) {
            ...
            const c1 = parseInt("0x" + c.join(""));
            return [(c1 >> 16) & 255, (c1 >> 8) & 255, c1 & 255];
        }
